...
| live RPC 1시간 Airflow run | VERIFIED | `airflow/logs/` 기준 successful scheduled run 반환값 33건, 최신 `row_count_after=6082932`, `dbt.returncode=0` 확인 |
...
목표: 남아 있는 미체크 항목이 실제 미구현·미검증 범위인지 확인하고, 이미 검증된 항목은 Airflow 로그와 storage metadata 근거에 연결합니다.
...
| 대상 | 확인 결과 | 해석 |
|---|---|---|
| `data/delta/ethereum_logs_v2/_delta_log` | 최신 commit file `00000000000000000036.json`, latest write time `2026-06-22 18:24:53 KST` | local Delta table에 누적 commit history가 존재합니다. |
| `data/analytics/ethereum_analytics_v2.duckdb` | file size `699412480` bytes | dbt downstream 산출물이 로컬 파일로 존재합니다. |
...
| `src/notebooks/04_accumulated_pipeline_data_freshness_validation.ipynb` | 현재 canonical Delta/DuckDB 산출물과 Python data contract 비교 | 실행 완료, PARTIALLY VERIFIED |
...
| Command | Result |
|---|---|
| `/workspace/.venv/bin/python -c "import pandas, duckdb, deltalake, nbformat, nbclient"` in `workspace-dev` | notebook runtime dependency import 성공 |
| `nbclient` execution of `src/notebooks/03_fixture_etl_replay_idempotency_validation.ipynb` in `workspace-dev` | output 저장 완료, error 없음 |
| `nbclient` execution of `src/notebooks/04_accumulated_pipeline_data_freshness_validation.ipynb` in `workspace-dev` | output 저장 완료, error 없음 |
| Notebook JSON parse for `src/notebooks/*.ipynb` | 5개 파일 parse 성공, saved error output 없음 |
...
```text
canonical raw Delta: PARTIALLY VERIFIED
dbt downstream relations: VERIFIED
raw to silver freshness: PARTIALLY VERIFIED
```

해석: 현재 `data/delta/ethereum_logs`에는 구 schema(`address`, `data`, `block_timestamp`, `ingested_at`)가 남아 있고, 최신 `src/cryptoquant_pipeline/delta_writer.py` 계약(`contract_address`, `data_raw`, `data_uint256_*`, `*_utc`)과 완전히 일치하지 않습니다.
따라서 누적 로컬 산출물은 현재 code contract 기준으로 `VERIFIED`가 아니라 `PARTIALLY VERIFIED`로 기록합니다.
...
| 실제 accumulated Delta를 최신 schema로 migration/rebuild | NOT VERIFIED | live 재수집 또는 명시적 migration 결정이 필요하며, 검증 중 기존 로컬 데이터를 덮어쓰지 않았습니다. |
...
| [`data/imgs/task_02_02_image.png`](../data/imgs/task_02_02_image.png) | Airflow DAG grid | `ethereum_hourly_logs` grid에서 displayed runs 61, total success 47, total failed 14, first run start `2026-06-20 23:41:04 KST`, last run start `2026-06-22 17:00:00 KST`, max run duration `00:31:06` | scheduled/manual run 이력과 성공/실패 혼재를 확인함 | PARTIALLY VERIFIED | 실패 원인은 이 화면만으로 확인 불가. 성공 run이 최신 data contract까지 충족했는지도 별도 검증 필요 |
...
| [`data/imgs/task_02_04_image.png`](../data/imgs/task_02_04_image.png) | Airflow success DAG run list | success DAG run record count 47, `ethereum_hourly_logs` scheduled run들이 `2026-06-22` 여러 logical date에 성공으로 표시됨 | Airflow UI 기준 성공 run history가 존재함 | PARTIALLY VERIFIED | 성공 run이 최신 repository state의 코드로 실행됐는지, raw/dbt 산출물이 최신 schema인지 이 화면만으로 확정 불가 |
...
| Screenshot-only data contract correctness | NOT VERIFIED | screenshot은 UI metadata 증거이며 data contract 검증은 notebook/dbt/test 결과를 봐야 함. `04_accumulated_pipeline_data_freshness_validation.ipynb`는 현재 raw Delta schema mismatch를 `PARTIALLY VERIFIED`로 판정함 |
...
| `scripts/inspect_outputs.py` direct inspection | `delta_row_count=6082932`, `delta_duplicate_natural_key_count=0`, `erc20_transfers_row_count=5400325`, `tether_treasury_flow_row_count=2` |
| DeltaTable direct metadata | `version=36`, `file_count=37`, `row_count=6082932` |
| DuckDB downstream relation count | `erc20_transfers=5400325`, `tether_treasury_flow=2`, `tether_treasury_flow_quality_summary=1` |
...
erc20_transfers=5400325
tether_treasury_flow=2
tether_treasury_flow_quality_summary=1
```

대조 결과, 최신 Airflow task log의 `row_count_after=6082932`와 `data/delta/ethereum_logs_v2`의 Delta row count가 일치합니다.
DuckDB에는 downstream relation이 생성되어 있으며, `erc20_transfers` row count도 수집 규모와 연결됩니다.

기본 `data/delta/ethereum_logs` 경로에는 구 schema 1건이 남아 있습니다. 따라서 기본 경로의 notebook 04 판정은 계속 `PARTIALLY VERIFIED`로 유지합니다.
...
| Delta Lake 적재 누적성 | VERIFIED | `ethereum_logs_v2` row count `6082932`, 최신 task log `row_count_after=6082932` 일치 | 기본 `ethereum_logs` 경로는 구 schema |
| dbt downstream 산출 | VERIFIED | `ethereum_analytics_v2.duckdb`의 `erc20_transfers=5400325`, `tether_treasury_flow=2`, `quality_summary=1` | DuckDB view `ethereum_logs`는 `/opt/airflow/...` 절대 경로에 묶여 있어 컨테이너별 mount path 확인 필요 |
| Production-grade 지속 운영 안정성 | NOT VERIFIED | 로컬 Docker의 성공·실패 이력과 재시도 후 성공 run만 확인했습니다. | provider SLA, alerting, secret rotation, full-history backfill은 미검증 |
...
- Airflow DAG 역할: `run_interval()` callable 호출. 개별 dbt 모델명은 DAG 파일에 없음.
...
- hidden/ref 의존성 명시: model/test SQL 상단의 `-- depends_on: {{ ref(...) }}` 주석으로 dbt parser가 graph를 명확히 해석하게 함.

새 모델은 dummy model이 아니라 `tether_treasury_flow`의 현재 Airflow/dbt window 결과를
1행으로 요약하는 품질 view다. `flow_row_count`, `transfer_count`,
`direction_count`, inflow/outflow raw/USDT 합계를 노출한다.

### Airflow DAG 무수정 검증

이 검증 세션에서 Airflow DAG 파일은 수정하지 않았다. active DAG 파일 해시는 시작과
종료 시점 모두 동일했다.
...
작업트리에는 검증 시작 전부터 아래 Airflow 상태가 있었고, 검증 종료 시에도 동일했다.
이번 Bonus 검증에서는 Airflow 파일을 patch하지 않았다.
...
| Airflow 무수정 조건 | VERIFIED | active DAG SHA256 시작/종료 동일, 이번 세션에서 Airflow 파일 patch 없음 |
...
| Airflow UI run history | PARTIALLY VERIFIED | `data/imgs/` screenshot에서 DAG 등록, `@hourly`, success/failed history 확인. UI screenshot 단독으로는 row-level correctness 증거가 아님 |
...
레거시 삭제와 문서/경로 교정 후 현재 repository 상태에서 재실행한 검증이다.
...
1시간 E2E를 확인했다. 다만 이 결과는 로컬 Docker 실행 이력 기준이며, production
Airflow 배포, provider SLA, alerting, full-history backfill은 별도 검증 대상이다.
...
미검증 당시 항목: 이 2026-06-20 세션에서는 실제 provider에서
`transfer_topic_all_addresses` scope의 1시간 live E2E를 완료하지 못했다.
이후 2026-06-22 Airflow task log와 `ethereum_logs_v2` 산출물로 1시간 scheduled 수집
이력은 확인했다. PHASE 0/PHASE 5 provider qualification fingerprint 비교와
staging-to-canonical publication manifest는 여전히 구현하지 않았다.

## Docker Ubuntu/Python 통합 재검증

2026-06-19 KST에 개발/운영 Docker 이미지를 Ubuntu 24.04/Python 3.12 기준으로 재검증함.
...
Airflow 2.10.x와 dbt 1.9+는 `protobuf` 요구 범위가 충돌해 단일 Python 실행 경로로 빌드할 수 없었음.
따라서 같은 Docker 이미지 안에서 Airflow 실행 경로와 project/dbt 실행 경로를 분리하고, 양쪽 모두 `pip check`를 통과하는지 검증함.
...
2026-06-20 KST 총괄 검증에서 Airflow 전용 Python 경로로 재확인함. 프로젝트 Python에는 Airflow가 설치되지 않는 분리 구조이므로 `/opt/airflow/python/bin/python`을 사용해야 함.
...
현재 기준 의존성 설치는 호스트 Python이 아니라 Docker 이미지 빌드 단계에서 수행함.
...
최초 실패 3: DuckDB 1.5.4에 `from_base()` 함수 없음.
...
이 시점의 legacy selector는 이후 `tag:ethereum_hourly`로 교체했다. 현재 실행
명령은 아래 2026-06-20 이후 검증 블록을 기준으로 한다.
...
2026-06-20 KST 총괄 제출 검증에서는 새 `current` fixture를 생성해 동일 graph를 재검증함.
...
실수로 `--full-refresh`와 일반 build를 병렬 실행했을 때 DuckDB file lock이 발생함.

```text
Cannot open file "...ethereum_analytics.duckdb": 다른 프로세스가 파일을 사용 중
```

수정: dbt 명령은 같은 DuckDB 파일을 쓰므로 순차 실행함.

현재 full-refresh 재검증은 이 세션에서 수행하지 않았다. 최신 검증은 fixture 기반
`dbt build --select tag:ethereum_hourly` 결과를 기준으로 한다.
...
원인: 과거 호스트 Windows PATH에 `git` 명령이 없었음. 현재 컨테이너 이미지에는 `git`을 포함함.

### Airflow DAG parse/list

호스트 Python에서는 실행하지 않음. Airflow CLI/runtime은 Ubuntu 24.04 Docker 이미지 안에서만 검증함.

### 실제 RPC

2026-06-20 KST에 `src/.env`의 legacy provider 키 이름(`CHAINSTACK_*`)을 fallback으로 읽어 실제 RPC 일부를 검증함. endpoint, key, username, password 원문은 출력하지 않음.
...
최신 finalized 10블록 smoke는 당시 USDT address-filtered `eth_getLogs`로 수행했음.
현재 acceptance scope는 `transfer_topic_all_addresses`이므로 이 결과 자체는 provider 연결
smoke로만 사용함. 2026-06-22 Airflow task log의 scheduled run 결과가 현재 CORE live E2E
evidence 역할을 함:
...
해석: 현재 연결된 provider는 최신 finalized 조회와 최신 10블록 log 조회는 허용하지만, binary search가 요구하는 임의 과거 block lookup을 막음. Alchemy Free 등 과거 block lookup을 허용하는 endpoint로 `src/notebooks/01_rpc_provider_connection_smoke_test.ipynb`와 `src/notebooks/02_eth_getlogs_transfer_sample_validation.ipynb`부터 재실행 필요.
...
아래 검증은 새 dependency 설치나 Docker image rebuild 없이, 이미 존재한 `workspace-dev` image를 사용해 수행했다.
...
| `docker compose config` | exit 0. Docker client config 접근 경고 있음 |
| `docker compose -f docker-compose.yaml -f .devcontainer/docker-compose.devcontainer.yaml images workspace-dev` | 기존 image 존재 확인 |
...
| `docker compose -f docker-compose.yaml -f .devcontainer/docker-compose.devcontainer.yaml run --rm --no-deps workspace-dev ruff check .` | `All checks passed!` |
| `docker compose -f docker-compose.yaml -f .devcontainer/docker-compose.devcontainer.yaml run --rm --no-deps workspace-dev dbt parse --project-dir dbt --profiles-dir dbt --no-partial-parse` | 최초 실행은 `models.ethereum_analytics.tests` unused configuration path warning 있음. `dbt/dbt_project.yml` 수정 후 재실행 결과 exit 0, 경고 없음 |
