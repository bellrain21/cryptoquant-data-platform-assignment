...
| ASSIGNMENT_DIRECT | 미해결 항목과 시도한 접근 방식 | `README.md`, `docs/05_validation_evidence.md`, `docs/07_submission_readiness_report.md` | provider limitation, scheduler run, canonical reorg replacement 미구현을 숨기지 않았습니다. | 검증 결과 문서와 상태 라벨 대조 | VERIFIED | 상태는 새 검증 결과에 따라 갱신해야 합니다. |
...
| DERIVED_CORE | Reorg 최소 대응 | `src/cryptoquant_pipeline/block_range.py`, `docs/04_failure_retry_backfill_strategy.md` | finalized head 이후 interval만 처리하고 raw `block_hash`, `removed`를 보존합니다. | unit tests, 문서 확인 | PARTIALLY VERIFIED | canonical replacement와 common ancestor reconciliation은 미구현입니다. |
| RELEASE | 실제 외부 RPC 1시간 end-to-end | `airflow/logs/dag_id=ethereum_hourly_logs`, `data/delta/ethereum_logs_v2`, `data/analytics/ethereum_analytics_v2.duckdb` | Airflow scheduled run이 외부 RPC block range를 수집하고 Delta 적재와 dbt build를 수행했습니다. | successful scheduled run 반환값 33건, 최신 `row_count_after=6082932`, DuckDB `erc20_transfers=5400325` | VERIFIED | 로컬 Docker 실행 이력 기준입니다. provider SLA, full-history backfill, production monitoring은 별도 검증 대상입니다. |
...
| OPTIONAL | 노트북 기반 코드·데이터 흐름 검증 | `src/notebooks/03_fixture_etl_replay_idempotency_validation.ipynb`, `src/notebooks/04_accumulated_pipeline_data_freshness_validation.ipynb` | 노트북을 current Python source와 canonical Delta/DuckDB data freshness를 확인하는 보조 증거로 정리했습니다. | `nbclient` 실행, notebook JSON parse, 저장된 output 확인 | PARTIALLY VERIFIED | 03번 fixture flow는 실행 완료. 04번은 기본 raw Delta가 최신 Python schema와 불일치한다고 판정합니다. |
