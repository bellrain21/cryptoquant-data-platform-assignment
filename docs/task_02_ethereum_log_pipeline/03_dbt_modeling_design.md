...
> 현재 구현 기준 dbt graph는 `ethereum_logs` -> `erc20_transfers` -> `tether_treasury_flow` -> `tether_treasury_flow_quality_summary`이며, 이 문서의 확장 모델 후보 전체가 구현된 것은 아니다.

> **문서 상태(Status)**: Legacy draft / 구현 전 확장 설계 메모
> **문서 역할(Role)**: canonical source, token metadata dimension, 별도 netflow 모델 후보를 정리한다. 현재 실행 기준은 `dbt/models/staging/ethereum_logs.sql`, `dbt/models/silver/erc20_transfers.sql`, `dbt/models/gold/tether_treasury_flow.sql`, `dbt/models/gold/tether_treasury_flow_quality_summary.sql`임.
