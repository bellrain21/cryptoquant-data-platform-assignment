...
주의: 아래 문서에는 Bronze/Silver canonical 분리, token metadata dimension, 별도 `tether_treasury_netflow` 모델처럼 현재 코드에 구현되지 않은 후보 설계가 포함됩니다. 

사전 과제 제출 기준으로는 현재 구현된 raw Delta `ethereum_logs`, dbt `erc20_transfers`, `tether_treasury_flow`, fixture 검증 증거를 우선합니다.
...
| Airflow task log + Delta/DuckDB metadata | VERIFIED | `airflow/logs/`, `data/delta/ethereum_logs_v2`,<br>`data/analytics/ethereum_analytics_v2.duckdb` 대조로 외부 RPC scheduled 수집과 downstream 산출을 확인 |
