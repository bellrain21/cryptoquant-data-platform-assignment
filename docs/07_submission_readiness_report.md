...
| Caches | `__pycache__/`, `.pytest_cache/`, `.ruff_cache/`, `.mypy_cache/` | ignore 대상임 |
| Local environments | `.venv/`, `venv/` | ignore 대상임 |
| Local logs/config | `airflow/logs/`, `logs/`, `airflow.cfg`, `dbt/profiles.yml`, `dbt/.user.yml` | ignore 대상임 `profiles.yml.example`은 유지함 |
| Binaries/extensions | `data/duckdb_extensions/` | ignore 대상임 |
| Possible sensitive files | `.env`, `.env.*`, real RPC URLs, local generated config | ignore 대상임 `.env.example`만 유지함 |
...
| `tx_input` | 이전 output 소비 참조임 UTXO lifecycle 재구성에 필요 |
| `tx_output` | 새 spendable right 생성. gross output volume과 UTXO 생성 근거 |
| `utxo` | day-end supply 계산 또는 lifecycle 검증 보조임 현재 snapshot만으로 과거 supply를 단독 복원하지 않음 |
...
| Change output limitation | V1은 change output을 제거하지 않음 | 주소 군집화와 change heuristic이 제공되지 않아 투명한 gross output 기준을 선택함 |
...
|---|---|---|
| Raw log unique key chain identity | raw row에 `chain_id`와 `block_hash`는 저장됨 Delta dedup key는 `chain_id + transaction_hash + log_index`이며 `block_hash`는 포함하지 않음 | retry dedup은 구현함 reorg replacement key는 구현되지 않음 |
...
|---|---|
| External attacker | Local Airflow UI, exposed port, default credentials, weak auth를 악용가능 |
| Local developer/operator mistake | `.env`, RPC URL, generated logs/data를 Git에 추가하거나 잘못된 backfill을 실행가능 |
| Malicious or faulty RPC provider | rate limit, partial response, malformed payload, wrong chain endpoint, stale data를 반환가능 |
| Normal chain/data failure | Ethereum reorg, retry duplication, partial ingestion, Delta writer conflict, uint256 precision loss가 발생가능 |
...
| Ethereum reorg | stale raw/canonical rows | finality buffer, `block_hash` stored | canonical replacement 구현되지 않음 | checkpoint reconciliation, reorg lookback rebuild | P1 |
...
| Address label drift | wrong entity flow | env-configured address | provenance/version/expiry가 없음 | label registry with source, valid_from/to, confidence | P1 |
...
|---|---|---|
| `python -m pytest -q` | BLOCKED | 호스트 Python 3.13 환경에서 `No module named pytest`가 발생함 의존성 미설치로 차단됐으며, 코드 실패로 판정하지 않음 |
| `python -m compileall src tests airflow/dags scripts` | PARTIALLY VERIFIED | `src`, `tests`, `airflow/dags`, `scripts` 문법 compile 통과 |
| `$env:PYTHONPATH='src'; python -c "...PipelineConfig...EthereumIngestionSettings..."` | PARTIALLY VERIFIED | 핵심 설정 객체 import와 기본 env parse는 통과함 Airflow/dbt/Delta optional dependency import는 검증하지 않음 |
| `docker compose -f docker-compose.yaml -f .devcontainer/docker-compose.devcontainer.yaml config --quiet` | VERIFIED | exit 0 |
| `dbt parse --project-dir dbt --profiles-dir dbt --no-partial-parse` | BLOCKED | 호스트 환경에 `dbt` executable이 없음 |
| `ruff check .` | BLOCKED | 호스트 환경에 `ruff` executable이 없음 |
...
|---|---|---|
| High | Real Ethereum RPC/mainnet ingestion은 로컬 Docker에서 검증됐지만 production SLA는 검증하지 않음 | Airflow log, Delta/DuckDB evidence로 검증 범위를 명시하고 production hardening은 별도 처리 |
| High | Reorg canonical replacement not implemented | finality buffer only로 문서화 |
| Medium | Multi-provider chain identity/cross-check 구현되지 않음 | 단일 provider `eth_chainId` 일치 확인만 구현 |
...
| Medium | Airflow UI/default credentials are local demo settings | production security control이 아니라는 점을 문서화함 |
...
| 7 | 2026-06-22 | reorg 한계 최종 정리 | 구현된 finality/idempotency와 구현되지 않은 canonical replacement가 README/report에 일치함 |
| 8 | 2026-06-22 | 최종 제출 검증 | 실제 실행한 명령과 Airflow log/Delta/DuckDB evidence만 VERIFIED로 갱신하고, production에서 검증하지 않은 항목은 숨기지 않음 |
