...
| 목표 | 처리 결과 |
|---|---|
| Airflow DAG의 orchestration 책임 유지 | DAG는 계속 `pipeline.run_interval()`만 호출함 |
| dbt 실행 책임 분리 | `src/cryptoquant_pipeline/dbt_runner.py`를 추가해 subprocess, selector, secret masking을 분리함 |
| 설정값과 chunking 연결 | `PipelineSettings.max_blocks_per_log_request`가 `collect_raw_logs()`에 명시적으로 전달됨 |
| SQL projection 명확화 | dbt model과 singular test의 `SELECT *`를 제거함 |
| 문서-코드 경로 정합성 | README, architecture, data contract, code reading guide, requirement matrix를 현재 경로와 맞춤 |

## 변경한 Python 파일

| 파일 | 변경 내용 | 요구사항 연결 |
|---|---|---|
| `src/cryptoquant_pipeline/dbt_runner.py` | dbt subprocess 실행, timeout, 로그 tail masking, `tag:ethereum_hourly` selector 실행 책임을 새 모듈로 분리함 | 신규 dbt 모델이 DAG 수정 없이 selector/ref graph로 반영되는 구조를 보존함 |
| `src/cryptoquant_pipeline/pipeline.py` | dbt 실행 구현을 제거하고 `run_dbt_build()` import만 사용하도록 정리함 | pipeline orchestration과 dbt runner 책임을 분리함 |
| `src/cryptoquant_pipeline/log_collector.py` | `max_blocks` 인자를 추가하고 설정값을 chunk 분할에 전달할 수 있게 함 | provider chunk 상한과 설정 계약의 연결성을 높임 |
| `tests/test_dbt_contracts.py` | dbt model/test source에서 `select *`를 금지하는 정적 회귀 테스트를 추가함 | SQL 품질 기준과 실패 row 추적 가능성을 고정함 |

## 변경한 SQL 또는 dbt 모델

| 파일 | 변경 내용 | 요구사항 연결 |
|---|---|---|
| `dbt/models/silver/erc20_transfers.sql` | `source_logs`, `typed_amounts`, `usdt_amount_text` CTE의 projection을 explicit column list로 교정함 | ERC-20 decoding grain과 incremental 입력 컬럼을 명확히 함 |
| `dbt/models/gold/tether_treasury_flow_quality_summary.sql` | upstream flow와 final summary projection에서 `SELECT *`를 제거함 | dbt graph 자동 반영 검증용 view의 출력 컬럼을 명확히 함 |
| `dbt/tests/erc20_transfer_integrity.sql` | 실패 row projection을 검사 대상 컬럼으로 제한함 | 실패 원인을 컬럼 단위로 추적함 |
| `dbt/tests/erc20_amount_numeric_status_integrity.sql` | 실패 row projection을 amount/status 컬럼으로 제한함 | uint256 numeric status 계약을 명확히 함 |
| `dbt/tests/ethereum_logs_uint256_contract.sql` | 실패 row projection을 raw uint256 계약 컬럼으로 제한함 | raw Delta uint256 보존 계약을 검증함 |
| `dbt/tests/non_usdt_amount_usdt_null.sql` | 실패 row projection을 USDT 파생 컬럼으로 제한함 | non-USDT row에 USDT decimals가 적용되지 않도록 검증함 |
| `dbt/tests/treasury_flow_integrity.sql` | 실패 row projection을 Treasury flow key/amount 컬럼으로 제한함 | hour/direction grain과 aggregate 계약을 검증함 |
| `dbt/tests/usdt_amount_numeric_not_null.sql` | 실패 row projection을 USDT numeric 필수 컬럼으로 제한함 | USDT row의 numeric 변환 실패를 dbt build 실패로 처리함 |

## 변경한 Notebook 파일

| 파일 | 변경 내용 | 요구사항 연결 |
|---|---|---|
| `src/notebooks/00_notebook_validation_index.ipynb` | 노트북 실행 순서, 외부 RPC 실행하지 않은 경계, 검증 목적을 정리함 | 검토 시 노트북을 학습 기록이 아니라 검증 보조 증거로 읽으실 수 있게 함 |
| `src/notebooks/01_rpc_provider_connection_smoke_test.ipynb` | 현재 `PipelineSettings`, `EthereumJsonRpcClient` 기준으로 provider 연결 smoke를 재정렬하고 secret 출력 금지 경계를 명시함 | 실제 RPC credential이 있을 때만 연결 검증을 수행함 |
| `src/notebooks/02_eth_getlogs_transfer_sample_validation.ipynb` | 현재 decoder helper와 `Transfer` topic0 기준 sample 검증 흐름으로 정리함 | `eth_getLogs` sample 검증과 외부 RPC 비용 경계를 분리함 |
| `src/notebooks/03_fixture_etl_replay_idempotency_validation.ipynb` | fixture → normalizer → decode → Delta 재실행 멱등성 흐름을 현재 package 기준으로 갱신하고 실행 output을 저장함 | Python source code와 Delta idempotency 검증 근거를 노트북에서 직접 확인가능 |
| `src/notebooks/04_accumulated_pipeline_data_freshness_validation.ipynb` | Delta/DuckDB 후보 인벤토리, 최신 v2 pair 선택, DB 추출 DataFrame, 시간대별 적재 추이, freshness, hourly gap을 확인하는 판정형 노트북으로 정리함 | 로컬 accumulated data의 최신성·시계열 gap·DuckDB path 이식성을 `PARTIALLY VERIFIED`로 드러냅니다 |

## 변경 전 문제점

| 문제 | 영향 | 처리 |
|---|---|---|
| `pipeline.py`가 dbt subprocess 구현까지 포함 | pipeline callable의 책임이 넓어져 Airflow orchestration, 수집, dbt 실행 경계가 흐려졌음 | `dbt_runner.py`로 분리함 |
| `PipelineSettings.max_blocks_per_log_request`가 collector 호출에 직접 전달되지 않음 | 설정 계약과 실제 chunking 호출 사이의 추적성이 약함 | `collect_raw_logs(..., max_blocks=...)`로 연결함 |
| dbt model/test에 `SELECT *`가 남아 있었음 | 모델 grain과 실패 row 증거가 컬럼 단위로 추적되지 않음 | explicit projection과 정적 테스트를 추가함 |
| docs가 새 리팩토링 산출물을 가리키지 않았음 | README와 문서 목차만 읽으면 변경 근거를 찾기 어려웠음 | refactoring/documentation consistency report 링크를 추가함 |
| `src/notebooks/`에 중복 누적 검증 파일과 stale 경로가 섞여 있었음 | 어떤 노트북이 현재 코드·데이터 검증 기준인지 판단하기 어려웠음 | active notebook 5개만 번호화하고 03·04번 실행 output을 최신 상태로 저장함 |
...
| 범위 | 주석 기준 |
|---|---|
| `dbt_runner.py` | dbt selector가 DAG 모델명 하드코딩을 대체한다는 불변식을 남김 |
| `log_collector.py` | collection scope와 chunk 상한을 함께 적용하는 목적을 짧게 남김 |
| dbt SQL | `SELECT *` 제거 후 projection이 어떤 grain과 실패 증거를 남기는지 주석을 유지함 |
...
| 항목 | 상태 | 이유 | 대체 검증 |
|---|---|---|---|
| 실제 외부 RPC 1시간 수집 | VERIFIED | Airflow task log에서 successful scheduled run 반환값 33건과 v2 direct inspection을 확인함 | Delta/DuckDB direct inspection |
| Airflow scheduler/UI scheduled run history | VERIFIED | screenshot과 task log를 함께 확인함 UI screenshot 단독 증거가 아니라 task 반환값과 산출물 row count를 대조함 | DagBag import, screenshot evidence, Airflow log evidence |
| 최신 schema 기준 Airflow end-to-end 재실행 | PARTIALLY VERIFIED | `data/delta/ethereum_logs_v2`가 최신 schema fields와 row count `6848937`건을 가지지만 2026-06-22 12:00 UTC hourly gap이 남아 있음 | Delta direct inspection, notebook 04 |
| canonical reorg replacement | NOT VERIFIED | 현재 구현 범위가 finality buffer와 raw `block_hash` 보존까지임 | 문서에서 future hardening으로 분리 |
| multi-provider cross-check | NOT VERIFIED | 단일 provider 설정만 구현함 | `eth_chainId` mismatch 실패 처리 |

## 남은 기술 부채

| 우선순위 | 항목 | 영향 |
|---|---|---|
| P1 | block-hash checkpoint와 common ancestor 기반 reorg replacement | 장기 reorg에서 stale raw/canonical 상태 교정이 자동화되지 않음 |
| P1 | Airflow pool 또는 file lock 기반 writer 동시성 통제 | `max_active_runs=1` 외의 Delta writer lock은 없음 |
| P1 | provider qualification manifest | provider별 historical block lookup 제한을 실행 전에 기록하지 못함 |
| P2 | label provenance registry | Tether Treasury address label의 source/version/validity를 구조화하지 않음 |
| P2 | dependency lock/hash 검증 | requirements pin은 있으나 lockfile hash 검증은 없음 |
