...
> **문서 역할(Role)**: Network Velocity, Transaction Volume, Circulating Supply, 해석 범위의 객체 경계를 정의합니다.

## 2.1 Network Velocity 정의(Network Velocity Definition)

Network Velocity는 특정 기간 동안 관측된 Bitcoin 이동량(Flow)을 특정 시점의 공급량(Stock)으로 나눈 회전율형 온체인 지표입니다.
...
이 값은 물리적 속도가 아니라 공급량 대비 이동 활동의 상대적 크기를 나타냅니다.

### 2.1.1 CryptoQuant 공개 제품 참조(Product Reference)

CryptoQuant 공개 API는 Bitcoin Velocity를 아래와 같이 설명합니다.
...
API의 `window=day`, `window=hour`, `window=block`은 반환 데이터의 시간 해상도이며, 공개 정의의 후행 1년 분자를 하루치 분자로 바꾸는 뜻으로 해석하지 않습니다.

### 2.1.2 과제 전용 지표(Assignment-specific Metric)

과제는 공급량 정책을 직접 정의하도록 요구합니다. 또한 제품 내부의 `estimated transaction volume` 추정 규칙이 제공되지 않았습니다. 따라서 본 과제의 기본 지표는 아래와 같이 별도로 정의합니다.
...
두 값 모두 CryptoQuant 공개 제품의 수치와 동일하다고 주장하지 않습니다.

## 2.2 Transaction Volume 정의(Transaction Volume Definition)

Bitcoin 거래는 기존 UTXO를 입력으로 소비하고 새 출력을 생성합니다. 일반 거래는 수취인에게 전달되는 출력 외에 잔여 금액을 송신자 측에 되돌리는 Change Output(거스름 출력)을 포함할 수 있습니다.

따라서 아래 두 개념은 동일하지 않습니다.
...
본 과제 V1은 주소 군집화, Change Address Heuristic, Entity Label을 사용하지 않습니다. 따라서 분자는 아래처럼 정의합니다.
...
V1 분자는 Change Output과 Self-churn(동일 주체 내부 이동)을 포함할 수 있습니다. 따라서 아래 표현은 사용하지 않습니다.
...
V1 분자는 **원천 온체인 데이터만으로 재현 가능한 Gross On-chain Output Volume**입니다.

## 2.3 Circulating Supply 정의(Circulating Supply Definition)

Circulating Supply는 단일 프로토콜 필드가 아닙니다. 본 과제는 발행 총량, 정책상 포함 가능한 UTXO 공급량, 장기 비활성 조정 공급량을 분리합니다.
...
이는 발행량 관점의 Stock이며, 현재 UTXO가 정책상 분모에 포함되는지와는 다른 객체입니다.

### 2.3.2 Policy-eligible UTXO Supply(정책상 포함 가능한 UTXO 공급량)

본 과제의 기본 분모는 아래와 같습니다.
...
`policy-eligible`은 모든 지갑·스크립트 조건에서 즉시 소비 가능함을 증명한다는 뜻이 아닙니다. 본 과제의 V1 정책상 유통 공급량 계산에 포함된다는 뜻입니다.
...
`dormant_threshold_days`는 원천 사실이 아니라 정책 파라미터입니다.
...
Dormant UTXO는 오랜 기간 소비되지 않았다는 관측 상태일 뿐 영구 분실 코인이라는 판정이 아닙니다.
...
이 값은 장기 비활성 물량을 분모에서 제외했을 때의 유동성 민감도 분석을 위한 보조 분모다. 절대적 유통 공급량의 유일한 정답으로 취급하지 않습니다.

### 2.3.5 Burn 처리(Burn Treatment)

V1은 원천 온체인 데이터로 판별 가능한 소비할 수 없는 출력만 기본 공급량에서 제외합니다.
...
외부 라벨만으로 burn address라고 부르는 주소, 개인키가 없다고 추정되는 주소는 V1에서 자동 제외하지 않습니다. 이들은 외부 큐레이션과 신뢰도 정책이 필요한 대상이며 향후 버전 관리되는 burn registry로 분리합니다.
...
이 값은 특정 시점에 남아 있는 공급량이 아니라 특정 기간에 발생한 이동량이다. 따라서 `dormant_reactivated_supply`라는 이름을 사용하지 않습니다.
...
이 결론에는 주소 라벨, 거래소 흐름, Change Output 처리, 시장 가격·유동성 등 별도 데이터가 필요합니다.
