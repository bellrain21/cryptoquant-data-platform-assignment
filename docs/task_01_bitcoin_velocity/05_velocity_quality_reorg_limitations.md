...
> **문서 역할(Role)**: Reorg로 인한 지표 영향·재계산 범위를 정의하고, V1의 한계와 확장 방향을 명시합니다.

## 12. Chain Reorganization 대응 설계(Chain Reorganization Handling)

## 12.1 Reorg가 Velocity에 미치는 영향

Reorg는 기존에 선택된 Best Chain 일부가 다른 branch로 교체되는 상황입니다. 이 경우 다음 값이 변할 수 있습니다.
...
- affected date 이후의 day-end UTXO supply가 계속 달라질 수 있습니다.
```

따라서 Reorg 영향은 단지 최근 하루의 분자만 다시 계산하는 문제가 아닙니다.

## 12.2 Best Chain 기준 처리

이 설계에서 `best_chain`은 특정 `observed_at` 시점에 파이프라인이 선택한 체인 경로입니다.
...
저장된 checkpoint와 현재 체인 상태를 비교합니다.
...
불일치가 발생하면 현재 tip에서 `previous_block_hash`를 역추적해 가장 최근 공통 조상(Common Ancestor)을 찾습니다.
...
Reorg 감지와 별도로 최근 N개 블록 또는 최근 N일을 주기적으로 다시 대조합니다.
...
둘은 같은 개념이 아닙니다. 최근 재검증 창은 감지 효율을 위한 운영 정책이고, 실제 복구 범위는 Reorg가 확인된 체인 상태에서 결정합니다.

## 13. 한계점(Limitations)

## 13.1 Gross On-chain Output Volume의 과대계산 가능성

V1 분자는 Change Output과 Self-churn을 제거하지 않습니다. 따라서 경제적 실질 이전량보다 크게 관측될 수 있습니다.

## 13.2 Policy-eligible UTXO Supply의 정책 의존성

이 분모는 프로토콜 표준의 유일한 circulating supply가 아니라, V1 정책상 포함 가능한 UTXO의 합입니다. burn 분류, maturity, dormant threshold 변경은 수치를 바꿉니다.

## 13.3 Dormant 기준의 불확실성

장기 미사용은 영구 분실을 증명하지 않습니다. dormant threshold는 유동성 민감도 분석을 위한 정책이며, 해석 시 버전과 기준을 반드시 함께 표시해야 합니다.

## 13.4 Block Header Timestamp의 한계

`metric_date`는 UTC 기준 block header timestamp를 사용한 보고 규칙이다. 이 값은 각 거래가 실제로 발생한 정확한 wall-clock time을 보장하지 않습니다.

## 13.5 원천 `utxo` 테이블의 이력 보장 여부

현재 상태만 가진 UTXO 테이블로는 과거 day-end supply를 복원할 수 없습니다. historical snapshot 또는 `tx_input`·`tx_output` 기반 lifecycle 재구성이 필요합니다.

## 13.6 온체인 지표 단독 해석의 한계

Velocity는 가격 방향, 매수·매도, 거래소 흐름, 특정 기관 활동을 직접 확정하지 않습니다. 이 해석에는 entity label, exchange flow, 가격·유동성, 외부 이벤트가 추가로 필요합니다.
...
주소 군집화와 entity label을 별도 차원 테이블로 관리하면 거래소·채굴자·기관 내부 이동을 분리한 지표를 만들 수 있습니다. 이 단계부터는 라벨 출처·신뢰도·유효기간이 필수 메타데이터입니다.

## 14.3 Versioned Burn Registry

외부 검증을 거친 burn address 또는 script registry를 별도로 도입할 수 있습니다.
...
상장 상태, 입출금 중단, 유동성, 시장 가격을 기본 온체인 지표와 분리해 결합합니다. 기본 지표의 재현성을 유지하면서 해석력을 높이는 방식입니다.

## 14.5 다중 자산 지표 템플릿

자산별 UTXO 모델과 account model의 차이를 고려해 공통 운영 원칙만 재사용합니다.
